...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const noCutSuffix = " CRITICAL RULES: This is ONE SINGLE CONTINUOUS UNBROKEN CAMERA TAKE with ZERO cuts. The camera NEVER stops. The screen NEVER goes dark, black, or dim - not even for a single frame. There are NO fade-to-black transitions, NO dissolves, NO wipes, NO scene changes. The brightness stays consistent and high throughout the ENTIRE video. Every single frame shows a well-lit interior. The camera physically travels through real walkable space without any interruption whatsoever. HARD ARCHITECTURE RULE: walls are solid barriers. The camera may ONLY leave or enter a room through a real visible doorway, open passage, or hallway. NEVER pass through a wall, break a wall, erase a wall, morph a wall into a doorway, clip through corners, or teleport between rooms. If the destination room is not directly visible from the first room, the camera must travel through a visible hallway/corridor connection.";

    const defaultPrompt = "Smooth steadicam walkthrough of a home interior. The camera starts in the first room, turns toward a real visible doorway, exits only through that doorway, follows the natural apartment path through an open passage or bright hallway corridor if needed, and enters the next room only through its doorway. The rooms are connected by normal apartment circulation, not by broken walls. Walls, corners, doorframes, ceiling, and floor remain visible and physically consistent for the entire route. Bright warm natural daylight fills both rooms. The hallway or transition area between rooms is well-lit and visible. Photorealistic interior design showcase. The camera movement is fluid, slow, and cinematic - never stopping, always revealing more of the space.";
