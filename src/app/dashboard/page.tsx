        ...
        {isPremium && (
          <div className="mb-8">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              ...
              {/* Visualize */}
              <Link 
                href={(hasVisionSub || hasLocalVision) ? "/visualize" : "/checkout-vision"}
                className="group border border-gray-200 rounded-2xl p-6 hover:border-gray-400 hover:shadow-lg transition-all"
              >
                <div className="w-12 h-12 bg-gray-100 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                  <svg className="w-6 h-6 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span className={`inline-block mt-3 text-xs group-hover:underline ${(hasVisionSub || hasLocalVision) ? 'text-emerald-600' : 'text-amber-600'}`}>
                  {(hasVisionSub || hasLocalVision) ? 'צור הדמיה ←' : 'נדרש מנוי AI Vision'}
                </span>
              </Link>
