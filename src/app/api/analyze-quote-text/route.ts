...
אחר כך דוח משא ומתן קצר ומעשי:

סיכום
[2-3 משפטים: האם ההצעה נראית סבירה ומה הסיכון המרכזי]

השוואת מחירים
- [פריט]: [מחיר] → שוק [טווח] = [הערכה והסבר קצר]

סעיפים חסרים או כלליים מדי
- [מה חסר בהצעה: כמות/חומר/פינוי/אחריות/מע״מ/לוח זמנים]
...
- [ניסוח קצר של בקשה לתיקון/פירוט]

המלצה
[מה לעשות לפני חתימה]

חשוב: מקסימום 320 מילים אחרי ה-VERDICT. אל תמציא מחירי שוק אם אין התאמה בנתונים; במקרה כזה כתוב שצריך פירוט נוסף.`;
    ...
    return NextResponse.json({ analysis, verdict, giftClaim });
  } catch {
